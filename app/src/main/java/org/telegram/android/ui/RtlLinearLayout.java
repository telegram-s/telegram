 ...
 * <p/>
 * <p>
 * Also see {@link RtlLinearLayout.LayoutParams android.widget.LinearLayout.LayoutParams}
...
public class RtlLinearLayout extends ViewGroup {
    ...
    public RtlLinearLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.RtlLinearLayout, defStyle, 0);

        int index = a.getInt(R.styleable.RtlLinearLayout_orientation, -1);
        if (index >= 0) {
            setOrientation(index);
        }

        index = a.getInt(R.styleable.RtlLinearLayout_gravity, -1);
        if (index >= 0) {
            setGravity(index);
        }

        boolean baselineAligned = a.getBoolean(R.styleable.RtlLinearLayout_baselineAligned, true);
        if (!baselineAligned) {
            setBaselineAligned(baselineAligned);
        }

        mWeightSum = a.getFloat(R.styleable.RtlLinearLayout_weightSum, -1.0f);
...
//                 a.getInt(com.android.internal.R.styleable.LinearLayout_baselineAlignedChildIndex, -1);
        ...
        mShowDividers = SHOW_DIVIDER_NONE;//a.getInt(R.styleable.LinearLayout_showDividers, SHOW_DIVIDER_NONE);
    ...
    public int getBaseline() {
        ...
        RtlLinearLayout.LayoutParams lp = (RtlLinearLayout.LayoutParams) child.getLayoutParams();
    ...
    void measureVertical(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (useLargestChild &&
            ...
            for (int i = 0; i < count; ++i) {
                ...
                final RtlLinearLayout.LayoutParams lp = (RtlLinearLayout.LayoutParams)
        ...
        if (delta != 0 && totalWeight > 0.0f) {
            ...
            if (useLargestChild && heightMode != MeasureSpec.EXACTLY) {
                for (int i = 0; i < count; i++) {
                    ...
                    final RtlLinearLayout.LayoutParams lp =
                            (RtlLinearLayout.LayoutParams) child.getLayoutParams();
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new RtlLinearLayout.LayoutParams(getContext(), attrs);
    }
    ...
    protected boolean checkLayoutParams(ViewGroup.LayoutParams p) {
        return p instanceof RtlLinearLayout.LayoutParams;
    }

    @Override
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        super.onInitializeAccessibilityEvent(event);
        event.setClassName(RtlLinearLayout.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(RtlLinearLayout.class.getName());
    }
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            super(c, attrs);
            TypedArray a =
                    c.obtainStyledAttributes(attrs, R.styleable.RtlLinearLayout_Layout);

            weight = a.getFloat(R.styleable.RtlLinearLayout_Layout_layout_weight, 0);
            gravity = a.getInt(R.styleable.RtlLinearLayout_Layout_layout_gravity, -1);
